...
|---|---|---|---|---|---|
| Observation 1 / Section 3.4, pp. 13–14, Observation 1 and displayed derivatives | `observation1_public_complements_ai_substitutesSpec` -> `observation1_public_complements_ai_substitutes` | pending | pending | pending | `by sorry` |
...
1. Intake seal, once the complete statement review is current: `python3
   scripts/semantic_audit_reuse.py --paper AKO26KnowledgeCollapse --bootstrap-current
   --write`.
2. Proof loop: `lake build AKO26KnowledgeCollapse.PaperInterface` or the narrower touched
   proof module.
3. Freeze closeout presentation inputs once: final report, status, source map,
   ...
   --paper AKO26KnowledgeCollapse`; defer aggregate/site status to integration or release.
4. Closeout readiness: `python3 scripts/closeout_reuse_plan.py --paper AKO26KnowledgeCollapse`.
